...
os.chdir('/home/sidrah19220/nmd_review/comment7/m_tosn')
...
eud_orig.drop(c1, axis = 'columns', inplace = True)
c3 = ["IMT CELL"+str(i) for i in range(1,309)]
eud_orig.drop(c3, axis = 'columns', inplace = True)
...
c2 = ["IMMATURE CELL"+str(i) for i in range(1,536)]
#REMOVE iOSN AND tOSN ROWS FROM THE MATRIX AS ONLY mIOSNs ARE NEEDED IN ROWS
eud_orig.drop(c2, axis = 'index', inplace = True)
...
#TO MAKE MATURE AND TRANSITION AS MATURE
cell_mat={}
for key,value in all_ors.items():
    if key.startswith("IMT"):
        cell_mat[key]=[]
